...
#-------------------------------------------------------------------------
  ...
  #--------------------------------
    ...
    system.time(sales <- read.csv("../data/iowa.liquor.r.hw1.csv"
                                  , check.names = FALSE
    ...
    rm(sales)
    gc()
    system.time(sales1 <- fread("../data/iowa.liquor.r.hw1.csv")
                , gcFirst = TRUE
                )
...
#-------------------------------------------------------------------------
#---- Part 8: dcast() and melt()
#---- https://cran.r-project.org/web/packages/data.table/vignettes/datatable-reshape.html
#-------------------------------------------------------------------------		
  # Let's aggregate number of orders per day per segment
	years <- orders[, .(total.orders = .N
	                    )
	                , by = .(year(order.date), market)
	                ]
	# What if we want 3 columns "Consumer", "Home Office", "Corporate"
	# with corresponding number of orders, instead of 1 row per each?
	# This is an example of dcast - converting long table to wide table
	# To achieve the same result manually, one would need a LOT of code
  # dcast() function takes care of it for us
	years.dcast <- dcast(years
	                     , year ~ market # Defines id variable vs group variables
	                     , value.var = "total.orders" # Defines the variable with values to be split across groups
	                     )
	
	# You can also do it simulatenously with several value and/or group variables
	years1 <- orders[, .(total.orders = .N
	                    , total.sales = sum(sales)
	                    )
	                , by = .(year(order.date), segment)
	                ]
	years.dcast1 <- dcast(years1
	                     , year ~ segment
	                     , value.var = c("total.orders"
	                                     , "total.sales"
	                                     )
	                     )
	years2 <- orders[, .(total.orders = .N
	                    )
	                , by = .(year(order.date), segment, category)
	                ]
	years.dcast2 <- dcast(years2
	                     , year ~ segment + category
	                     , value.var = c("total.orders")
	                     )
	
	# dcast() is useful for creating variables to be used in regression analysis
	# But for things like ggplot() long tables are much more useful
	# because they allow a single variable to code multiple categories
	# So, how can we get back from a wide table to long?
	# Using melt():
	years.melt <- melt(years.dcast
	                   , id.vars = c("year")
	                   , measure.vars = c("Africa"
	                                      , "Asia Pacific"
	                                      , "Europe"
	                                      , "LATAM"
	                                      , "USCA"
	                   ...
	                   , value.name = "total.orders"
	                   )
	# Note that the result is not identical to years
	all.equal(years, years.melt)
	# That's because melt() by default coverts characters to factors
	years.melt[, market := as.character(market)]
	all.equal(years, years.melt)
	# Now it's different because of different ordering of rows
	setorder(years)
	setorder(years.melt)
	all.equal(years, years.melt) # Finally equal!
	
	# We can also do melt over multiple columns
	years.melt1 <- melt(years.dcast1
	                    , id.vars = c("year")
	                    , measure = list(c("total.orders_Consumer"
	                                       , "total.orders_Corporate"
	                                       , "total.orders_Home Office"
	                                       )
	                                     , c("total.sales_Consumer"
	                                         , "total.sales_Corporate"
	                                         , "total.sales_Home Office"
	                                         )
	                                     )
	                    , variable.name = "segment"
	                    , value.name = c("total.orders", "total.sales")
	                    )
	# And it also doesn't carry over segment names, only labels like "1", "2", "3"
	
	# Unfortunately, with two categories and one value variable it doesn't work all the way
	# You have to be careful with the order of names of measures and values
	years.melt2 <- melt(years.dcast2
	                    , id.vars = c("year")
	                    , measure = list(c("Consumer_Furniture"
	                                       , "Consumer_Office Supplies"
	                                       , "Consumer_Technology"
	                                       )
                                         , c("Corporate_Furniture"
                                             , "Corporate_Office Supplies"
                                             , "Corporate_Technology"
                                             )
                                         , c("Home Office_Furniture"
                                             , "Home Office_Office Supplies"
                                             , "Home Office_Technology"
                                             )
                                         )
                      , variable.name = "category"
                      , value.name = c("total.orders_Consumer"
                                       , "total.orders_Corporate"
                                       , "total.orders_Home Office"
                                       )
	                    )
	# We have segment variable, but category variable is still split over 3 columns
	# First replace category  with proper values
	years.melt2[, category := factor(category
	                                 , levels = c("1", "2", "3")
	                                 , labels = c("Furniture"
	                                              , "Office Supplies"
	                                              , "Technology"
	...
	# So we have to melt it again
	years.melt3 <- melt(years.melt2
	                   , id.vars = c("year", "category")
	                   , measure.vars = c("total.orders_Consumer"
                                       , "total.orders_Corporate"
                                       , "total.orders_Home Office"
                                       )
	                   , variable.name = "segment"
  ...
  # Again, replace segment values with proper names and change it to character to match original data
	years.melt3[, segment := gsub("total.orders_", "", segment)
	            ][, segment := as.character(segment)
	              ]
	# Reoder columns to match the order in years2
	setcolorder(years.melt3, names(years2))
	# Order rows to match each other
	setorder(years2)
	setorder(years.melt3)
	# And now they are equal
	all.equal(years2, years.melt3)
