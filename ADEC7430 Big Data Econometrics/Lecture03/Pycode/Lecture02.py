...
# does it make sense to have Age "float64"? (fractional)
# let's force pandas to read everything as a character
...
# OK we knew from above that the output will be sorted by (descending) frequency
...
# in such cases, resetting the (multi)index is best
rTrain.groupby(['Sex']).Age.value_counts().sort_index().reset_index(name='freq') 
# HW: from left to right, explain what the "chain" of commands above does
...
# !! Please note the difference between how we modify Pclass...
...
# HW Analyze and describe the distributions of Survived_j and Pclass_j
