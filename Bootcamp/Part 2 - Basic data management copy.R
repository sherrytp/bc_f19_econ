...
##-----------ClassNote on Feb. 15, 2019 -------------
##        Missing class on Feb. 22 & Mar. 1     
##---------------------------------------------------
##---------------------------------------------------
...
#-------------------------------------------------------------------------
  ...
  # Prepare needed libraries; have we already downloaded and restored all packages? 
  ...
  setwd("")   # Session - Set Working Directionary - Choose Working Directionary 
  ...
  # Alternatively, you could simply tell R where the file is by SELECTING and NEVAGATING thru pop-up windows
  orders <- read_excel(file.choose())
  warnings()
  # Alternatively, you could specify what values those data; integers or just textfile 
  orders <- read_excel("~/Downloads/sample.data.orders.xlsx", sheet = "Orders")
...
#-------------------------------------------------------------------------
  ...
  str(orders)
  # tbl_df and data.frame: "tibble" a joint result of dataframe and excel to one file. 
...
#-------------------------------------------------------------------------
#---- Step 4: data subsetting   --- missing class notes ------------------
#-------------------------------------------------------------------------
  ...
  identical(orders["sales"], orders[, "sales"])    # TRUE
  all.equal(orders["sales"], orders[, "sales"])    # TRUE 
  mean(orders["sales"]) # doesn't work because mean() can't take a dataframe as an argument
  mean(orders[, "sales"]) # works fine and get NA 
  ...
  colnames(orders)[19]
  View(orders[19])   # select a column by colnumber or colname 
  View(orders[, 19]) # same as View(orders[19]) or View(orders[,19]) 
  View(orders[, -19]) # Note that orders[, -"sales"] will not work; except column 19 
  orders[1:10, 6:10]
  
  # Extracting the values inside a column
  orders$shipping.cost
  orders$sales
  View(orders$sales)  # a mini dataframe of only numbers in sales 
  identical(orders$sales, orders[, "sales"])  # False, a tipple vs. a vector of values 
  str(orders$sales)
  str(orders[, "sales"])
  ...
  summary(orders[, "sales"])
  summary(orders["sales"])
  mean(orders$sales)    # a list/tipple of numbers 
  mean(orders[, "sales"])
  mean(orders["sales"])   # 加不加逗号一样, returning NA b/c a mini-dataframe 
  ...
  View(orders[, names(orders) %in% vars])   # view orders when colname = True; same as View(orders[,vars])
  View(orders[, !names(orders) %in% vars])   # view orders with all other columns 
  ...
  orders$bad.variable <- 1:51290    # fit in b/c data has 51290 
  
  # Create an empty the variable
  orders$empty.variable <- NA
  # This is NOT the same as
  orders$empty.string.variable <- ""
  # NA is a empty/blank missing variable but empty string is a space - not missing 
  ...
  colnames(orders)    # no row.id but has profit.per.item 
  
  # Select observations based on condition
  View(orders$profit > 0)  # returns True or False but all data in rows 
  ...
  View(orders[!orders$profit > 0     # same as orders$profit < 0 
              , c("sales", "profit", "quantity")
              ]
       )
  View(orders[! orders$profit > 0 & orders$quantity > 5
  ...
  View(orders[orders$profit > 0 | orders$discount > 0   # | means logical OR 
  ...
  orders$country
  countries <- unique(orders$country)   # just stores as the database initiates 
  countries
  length(countries)  # how many countries do we ship to 165. 
  
  # How can we get an alphabetical ordering of all our unique countries?
  order(countries)
  countries[c(6, 67, 80)]
  countries[c(1, 5, 8)]
  countries[order(countries)]  # what does this one do?!!!!!!!!!!!!!!!!!!!!!
  countries <- countries[order(countries)]    # by alphabetical order()
  countries
  
  # How many cities do we ship to? 3650. 
  length(unique(orders$city))
  nrow(unique(orders$city)) # doesn't work because nrow() expects a matrix/dataframe
  NROW(unique(orders$city)) # this works b/c only need a matrix consisting of one vector. 
  ...
  anyNA(orders$postal.code)  # - TRUE, has missing variables 
  ...
  View(orders[!is.na(orders$postal.code), "country"])  # countries with postal codes, opposite of missing 
...
#-------------------------------------------------------------------------  
  ...
  NROW(unique(orders[, c("city")]))    # doesn't matter using nrow or NROW 
...
#-------------------------------------------------------------------------  
  ...
  colnames(customers)[1] <- "c.id"     # just to show you how to merge when two colnames are different 
  ...
  customers1 <- merge(customers1, unique(orders[, c("customer.id"
                     ...
                     , by = c("customer.id")      # or use by = "customer.id" 
  ...
  all.equal(customers, customers1)  # !!!!!!!!!!!!? Mean relative difference: 0.6596661
  # the series number of two cases are different b/c the way we picked data 
  ...
  # Note that we can also avoid adding "weights" as extra variable into the data by using .alt()
  ...
  # So let's fix it by CREATING A FUNCTION that does is automatically for us
...
#-------------------------------------------------------------------------   
  ...
  strsplit("Diffent_ways to.split-words", " |_|.")   # "." happens????????????? 
  strsplit("Diffent_ways to.split-words", " |_|\\.")
  strsplit("Diffent_ways to.split-words", " |_|\\.|-")
  ...
  View(names[1])
  View(names[[1]])
  View(names[[1]][1])       # refers to Part1. indexing 
  View(sapply(names, function(x) x[1]))   # returns first names 
  View(sapply(names, function(x) x[2]))   # returns last names, but where did we define function???????????
