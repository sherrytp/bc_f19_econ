...
#----------------------------------------------
#----- ClassNote on Feb. 8 & Feb. 15, 2019 ----
#---six ppl seating, LYL LYN, XC XSN, NT WYF---
#----------------------------------------------
...
#-------------------------------------
  ...
  # What if we want to store the results somewhere?  rep means repeat.. 
...
#-------------------------------------
  ...
  3 == 3.0000000001
  3 == 3.000000000000000000001  # because R can only recognize 12 digits 
  ...
  dim(means.y)
  nrow(means.y)   # either matrix, dataframe, or lists 
  length(means.y)
  ...
  for (i in 1:length(means.y)) {
    if (means.y[i] > 5) { 
      ...
      text <- paste("Mean of column", i, "is", means.y[i], ", which is smaller than or equal to 5")
    }
    print(text)
  }
  
  # %in% operator    ---> similar to "in" in Python 
  2 %in% c(1, 3, 5)
  2 %in% c(1, 2, 5)
  c(1, 2) %in% c(1, 3, 5)
  c(1, 2) %in% c(1, 2, 5)
  ...
  which(c(1,2) %in% c(1, 3, 5))
  which(c(1,3,5,7,9) %in% c(5:10))
  c(1, 2)[which(c(1,2) %in% c(1, 3, 5))] 
...
#-------------------------------------
  ...
  # create y as a matrix 
  y <- cbind(1:6, 5:10)
  matrix.col.mean(y) # No result is returned
  means.y <- matrix.col.mean(y)   # save up the result to the global environment 
  # also this result is showing below lol 
